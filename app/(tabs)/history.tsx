...
export default function HistoryScreen() {
  ...
  useEffect(() => {
    if (version > 0) fetchHistory(false);
  ...
  const totalsSummary = useMemo(
    () =>
      items.reduce(
        (acc, it) => ({
          time: acc.time + it.total_time_minutes,
          money: acc.money + it.total_money_eur,
        }),
        { time: 0, money: 0 }
      ),
    [items]
      ...
      >
          ...
          <View>
            ...
            {!loading && (
              <Text style={{ color: '#666' }}>
                {totalPayouts > 0
                  ? `${totalPayouts} payout${totalPayouts > 1 ? 's' : ''} • Total: ${
                      totalsSummary.time
                    } min, ${fmtEUR(totalsSummary.money)}`
                  : 'No payouts yet'}
              </Text>
            )}
          </View>
        ...
        {empty && (
          ...
          >
            ...
            <Text style={{ color: '#666' }}>
              Confirm a payout in the Payout tab, then refresh.
            </Text>
          </View>
        )}
                  ...
                  >
                    ...
                    <View style={{ alignItems: 'flex-end' }}>
                      <Text style={{ color: '#111', fontWeight: '600' }}>
                        Time: {it.total_time_minutes} min
                      </Text>
                      <Text style={{ color: '#111', fontWeight: '600' }}>
                        Money: {fmtEUR(it.total_money_eur)}
                      </Text>
                    </View>
                  </View>
                      ...
                      {it.lines.map((l, idx) => (
                        ...
                        >
                          ...
                          <View style={{ flexDirection: 'row', gap: 12 }}>
                            <Text style={{ color: '#666' }}>
                              Time: {Number(l.time_minutes)} min
                            </Text>
                            <Text style={{ color: '#666' }}>
                              Money: {fmtEUR(Number(l.money_eur))}
                            </Text>
                          </View>
                        </View>
                      ))}
